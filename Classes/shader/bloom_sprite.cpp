...
bool BloomSprite::initWithFile(const char* filename)
{
	...
	// brightness shader
	//
	m_shader_brightness = new  CCGLProgram();  
	const char* fs = CCString::createWithContentsOfFile("shader/efx_brightness.fsh")->getCString();
	m_shader_brightness->initWithVertexShaderByteArray(ccPositionTextureA8Color_vert, (const GLchar*)fs);   
	
	CHECK_GL_ERROR_DEBUG();

	m_shader_brightness->addAttribute(kCCAttributeNamePosition, kCCVertexAttrib_Position);  
	m_shader_brightness->addAttribute(kCCAttributeNameColor, kCCVertexAttrib_Color);  
	m_shader_brightness->addAttribute(kCCAttributeNameTexCoord, kCCVertexAttrib_TexCoords);  

	CHECK_GL_ERROR_DEBUG();  
	m_shader_brightness->link();  
	CHECK_GL_ERROR_DEBUG();  
	m_shader_brightness->updateUniforms();  
	CHECK_GL_ERROR_DEBUG();  

	//
	// blur shader
	//
	m_shader_blur = new  CCGLProgram();  
	fs = CCString::createWithContentsOfFile("shader/efx_blur.fsh")->getCString();
	m_shader_blur->initWithVertexShaderByteArray(ccPositionTextureA8Color_vert, (const GLchar*)fs);   

	CHECK_GL_ERROR_DEBUG();

	m_shader_blur->addAttribute(kCCAttributeNamePosition, kCCVertexAttrib_Position);  
	m_shader_blur->addAttribute(kCCAttributeNameColor, kCCVertexAttrib_Color);  
	m_shader_blur->addAttribute(kCCAttributeNameTexCoord, kCCVertexAttrib_TexCoords);  

	CHECK_GL_ERROR_DEBUG();  
	m_shader_blur->link();  
	CHECK_GL_ERROR_DEBUG();  
	m_shader_blur->updateUniforms();  
	CHECK_GL_ERROR_DEBUG();  
...
#define kQuadSize sizeof(m_sQuad.bl)
	...
	m_frameTexture2->begin();
	{
		...
		glUniform2f(m_location_pix_size, 1.0f/m_tex_size.width, 0);
